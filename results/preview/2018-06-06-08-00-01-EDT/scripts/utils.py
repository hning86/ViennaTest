...
def test_tf_upload():
    ...
    dsvm = RemoteTarget('dsvm', 'haigpu.southcentralus.cloudapp.azure.com:5022', 'ninghai', 'Spark888*123')
    ws = Workspace.from_config()
    proj = Project.attach(ws, 'tf_model', '/tmp/tf_proj')
    shutil.copy('mnist_tf.py', proj.project_directory)
    shutil.copy('train-sklearn.py', proj.project_directory)
    proj.attach_legacy_compute_target(dsvm)
    rc = RunConfiguration.load(proj, 'dsvm')
    print(rc.target)
    #rc.environment.python.interpreter_path = '/home/ninghai/miniconda3/envs/tf/bin/python'
    rc.environment.python.user_managed_dependencies = False
    rc.prepare_environment = True
    from azureml.core.conda_dependencies import CondaDependencies
    cd = CondaDependencies()
    cd.add_conda_package('scikit-learn')
    cd.save_to_file(proj.project_directory, 'conda_dependencies2.yml')
    rc.environment.python.conda_dependencies_file = 'aml_config/conda_dependencies2.yml'
    rc.save()
    r = Run.submit(proj, rc, 'train-sklearn.py')
    print(helpers.get_run_history_url(r))
    r.wait_for_completion(show_output=True)

    r2 = Run(ws, 'tf_model', r.id)
    for f in r2.get_file_names():
        print(f)

def create_dsvm():
    from azureml.core.compute.dsvm import DsvmProvisioningConfiguration
    ws = Workspace.from_config()
    dpc = DsvmProvisioningConfiguration('stadard_nc6', 'ninghai')
    dsvm = ws.create_compute_target('gpu_dsvm', dpc)
    dsvm.wait_for_completion(show_output=True)

ws = Workspace.from_config()
print(ws.name)
for s in ws.list_webservices():
    print('delete', s.name)
    s.delete()
