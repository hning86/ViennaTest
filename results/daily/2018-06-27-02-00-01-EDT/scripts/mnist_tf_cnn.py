...
def main():
	...
	parser.add_argument('--minibatch_size', type = int, default = 50, help = 'minibatch size')
	parser.add_argument('--keep_probability', type = float, default = 0.5, help = 'keep probability for dropout layer')
	parser.add_argument('--num_iterations', type = int, default = 1000, help = 'number of iterations')
	parser.add_argument('--output_dir', type = str, default = './outputs', help = 'output directory to write checkpoints to')
	...
	for i in range(args.num_iterations):
		...
		sess.run(train_step, feed_dict={x: batch[0], y_: batch[1], keep_prob: args.keep_probability})
	...
	final_test_acc = sess.run(accuracy, feed_dict = {x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0})
